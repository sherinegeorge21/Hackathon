...
def sidebar(default_model):
    with st.sidebar:
        ...
        s = st.checkbox('tepper-tuned palm2 llm')
        if s:
            st.write('Fine Tuned LLM Palm 2 Selected')
            #selected_model='tunedModels/teppertrainedllm-2csdufxp0ssa'
        else:
        ...
        city = st.sidebar.multiselect("Select the Category:",
        options=['Academic','Faculty','Lifestyle','Admissions'],
        default="Academic"
)
        # st.markdown(
